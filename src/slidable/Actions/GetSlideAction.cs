...
namespace Slidable.Actions
{
    public class GetSlideAction
    {
        ...
        {
            _options = serviceProvider.GetRequiredService<IOptions<SlidableOptions>>().Value;
            _slidableClient = serviceProvider.GetRequiredService<ISlidableClient>();
            ...
            if (data.Values.TryGetInt("index", out int index))
            {
                ...
                if (show.TryGetSlide(index, out var slide))
                {
                    ...
                    var html = Embedded.Web.template_html.Utf8ToString()
                        ...
                        .Replace("{{slidable}}", $"slidable.io/live/{_options.Presenter}/{_options.Slug}");
        ...
        private async Task StartOnline(Show show)
        {
            ...
            try
            {
                _live = await _slidableClient.StartShow(new StartShow
                {
