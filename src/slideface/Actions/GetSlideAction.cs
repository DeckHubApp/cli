...
using SlideFace.Embedded;
using SlideFace.Rendering.Markdown;

namespace SlideFace.Actions
{
    public class GetSlideAction
    {
        private LiveShow _live;
        private readonly SlideFaceOptions _options;
        private readonly ISlideFaceClient _slideFaceClient;
        private readonly ILogger<GetSlideAction> _logger;

        public GetSlideAction(IServiceProvider serviceProvider)
        {
            _options = serviceProvider.GetRequiredService<IOptions<SlideFaceOptions>>().Value;
            _slideFaceClient = serviceProvider.GetRequiredService<ISlideFaceClient>();
            ...
            if (data.Values.TryGetInt("index", out int index))
            {
                ...
                if (show.TryGetSlide(index, out var slide))
                {
                    ...
                    var html = Embedded.Web.template_html.Utf8ToString()
                        ...
                        .Replace("{{slideface}}", $"slideface.com/live/{_options.Presenter}/{_options.Slug}");
        ...
        private async Task StartOnline(Show show)
        {
            ...
            try
            {
                _live = await _slideFaceClient.StartShow(new StartShow
                {
